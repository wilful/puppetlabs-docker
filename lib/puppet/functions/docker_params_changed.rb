...
module DockerCheckChanges
  class Base
    ...
    def has_changes?(opts, hash)
      ...
      inspect_volumes = hash['Config']['Volumes'].keys.sort if hash['Config'].key?('Volumes')
...
Puppet::Functions.create_function(:docker_params_changed) do
  ...
  def create_container(cmd, image)
    @console.pull_command(image)
    @console.run(cmd)
  end

  def restart_container(name, stop_wait_time, cidfile, cmd, image)
    @console.stop_command(name, stop_wait_time)
    @console.remove_command(name)
    @console.delete_command(cidfile)
    @console.pull_command(image)
    @console.run(cmd)
    @console.return_value = 'Param changed'
  end
  ...
  def detect_changes(opts)
    ...
    if opts['sanitised_title']
      ...
      if @console.status.success?
        ...
        restart_container(opts['sanitised_title'],
                          opts['stop_wait_time'],
                          opts['cidfile'],
                          opts['command'],
                          opts['image']
        ...
        create_container(opts['command'], opts['image']) unless File.exist?(opts['cidfile'])
        @console.inspect_command(opts['sanitised_title'])
        unless @console.status.success?
          @console.delete_command(opts['cidfile'])
          create_container(opts['command'], opts['image'])
        end
        @console.return_value = 'No changes detected'
      end
    end
